...
const heroImages = computed(() => {
    return prop.data.map((item) => item.images[0]).filter((item) => item != undefined)
    // return prop.data.map((item: any) => item.images[0]).filter((item: any) => item != undefined)
})
</script>
